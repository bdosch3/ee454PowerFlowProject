function [J_new] = createJacobian(x, Y, N, m, PV, f_comp, Vswing, thetaSwing)
    ...
    for j = 1:(2*N-m-1)
        ...
        elseif j <= (N-1) && k > (N-1)
            if j ~= (k-N+1)
                ...
                J_new(k,j) = pComp(k-N+m)-(V(k-N+m+1)^2)*(real(Y(k-N+m+1,j+1)));
            end
        % at J_12
        elseif j > (N-1) && k <= (N-1)
            if (j-N+1) ~= k
                ...
                J_new(k,j) = (pComp(k)/V(k+1)) + V(k+1)*(real(Y(k+1,j-N+m)));
            end
        % at J_22
        else
            if j ~= k
                J_new(k,j) = V(k-N+m+1)*((real(Y(k-N+m+1,j-N+m+1)))*(sin(theta(k-N+m+1)-theta(j-N+m+1)))... 
                            - (imag(Y(k-N+m+1,j-N+m+1)))*(cos(theta(k-N+m+1)-theta(j-N+m+1))));
            else
                J_new(k,j) = (qLoad(k-N+m)/V(k-N+m+1))-V(k-N+m+1)*(real(Y(k-N+m+1,j-N+m+1)));
            end
        end
    end
end
