    ...
    for j = 1:(2*N-2)
        ...
        if j <= (N-1) & k <= (N-1)
            if j ~= k
                ...
                J_new(k,j) = -qLoad(k+1)-(V(k+1)^2)*imag(Y(k+1,j+1));
            end
        % at J_21    
        elseif j <= (N-1) & k > (N-1)
            if j ~= k
                ...
                J_new(k,j) = pLoad(k+2-N)-(V(k+2-N)^2)*real(Y(k+2-N,j+1));
            end
        % at J_12
        elseif j > (N-1) & k <= (N-1)
            if j ~= k
                ...
                J_new(k,j) = (pLoad(k+1)/V(k+1))+V(k+1)*real(Y(k+1,j+2-N));
            end
        % at J_22
        else
            if j ~= k
                ...
                J_new(k,j) = (qLoad(k+2-N)/V(k+2-N))-V(k+2-N)*real(Y(k+2-N,j+2-N));
            end
        end
    end
end
